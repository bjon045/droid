...
public class BackgroundPanel extends Entity {
	...
	if (GameHolder.gameMode == GameMode.MAIN_GAME) {
	    // center border
	    g.rotate(getX(), getY(), 90);
	    horizontalBorder.draw(getX(), ((getY() - getWidth()) + charPanelXSize) - borderThickness, getHeight(), borderThickness);
	    g.rotate(getX(), getY(), -90);

	    // top tee
	    borderTee.draw((getX() + ((getWidth() - charPanelXSize) + (borderThickness / 2))) - halfTeeThickness, getY());

	    // bottom tee
	    g.rotate(getX(), getY(), 180);
	    borderTee.draw((((getX() - getWidth()) + charPanelXSize) - (borderThickness / 2)) - halfTeeThickness, getY() - getHeight());
	    g.rotate(getX(), getY(), -180);
	}
    }
    ...
    /**
     * Get the topleft pixel location of the main window. Used for map, char screen etc
     * 
     * @return
     */
