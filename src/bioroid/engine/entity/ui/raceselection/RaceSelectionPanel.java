...
public class RaceSelectionPanel extends Entity {
	...
	// Race
	StringEntity raceLabel = new StringEntity(getX() + FIRST_LEFT_OFFSET, getY() + (counter1 * PANEL_GAP), "Choose a Race:");
	entities.add(raceLabel);
	counter1++;

	for (Race raceType : ModelResources.races.getRaces()) {
	    StringEntity raceValue = new StringEntity(getX() + SECOND_LEFT_OFFSET, getY() + (counter1 * PANEL_GAP), raceType.getName());
		...
		if (CollectionUtils.isNotEmpty(raceType.getSubraces())) {
		    ...
		    StringEntity subraceLabel = new StringEntity(getX() + THIRD_LEFT_OFFSET, getY() + (counter2 * PANEL_GAP), "Choose a Sub-race:");
		    entities.add(subraceLabel);
		    counter2++;

		    // No subrace option
		    StringEntity noSubrace = new StringEntity(getX() + FORTH_LEFT_OFFSET, getY() + (counter2 * PANEL_GAP), "None");
			...
			StringEntity subraceTypeValue = new StringEntity(getX() + FORTH_LEFT_OFFSET, getY() + (counter2 * PANEL_GAP), subraceType.getName());
			subraceTypeValue.setListener(new SubraceSelectionListener(subraceType.getDescription(), subraceType));
	...
	// career
	int counter2 = 1;
	StringEntity careerLabel = new StringEntity(getX() + SIXTH_LEFT_OFFSET, getY() + (counter2 * PANEL_GAP), 200, 100, "Choose a Career:");
	entities.add(careerLabel);
	counter2++;

	for (Career careerType : ModelResources.careers.getCareers()) {
	    StringEntity careerValue = new StringEntity(getX() + SEVENTH_LEFT_OFFSET, getY() + (counter2 * PANEL_GAP), careerType.getName());
	    careerValue.setListener(new CareerSelectionListener(careerType.getDescription(), careerType));

	    if (!careerType.canTakeCareer(pc)) {
		careerValue.setColour(Fonts.DARK_GREEN);
	    }
	...
	// top labels
	StringEntity attributeLabel = new StringEntity(getX() + FIRST_LEFT_OFFSET, getY() + (counter1 * PANEL_GAP), "Attributes:");
	...
	StringEntity totallabel = new StringEntity(getX() + FIFTH_LEFT_OFFSET, getY() + (counter1 * PANEL_GAP), "Total:");
	...
	for (Attribute attribute : ModelResources.attributes.getAttributes()) {
	    // attribute row
	    StringEntity attributeItemLabel = new StringEntity(getX() + SECOND_LEFT_OFFSET, getY() + (counter1 * PANEL_GAP), attribute.getName());
	    ...
	    StringEntity attributeRaceValue = new StringEntity(getX() + THIRD_LEFT_OFFSET, getY() + (counter1 * PANEL_GAP), String.valueOf(total - spentPoints));
	    entities.add(attributeRaceValue);

	    // total value

	    StringEntity totalValue = new StringEntity(getX() + FIFTH_LEFT_OFFSET, getY() + (counter1 * PANEL_GAP), String.valueOf(total));
	    // totalValue.setDrawBorder(true);
	    entities.add(totalValue);

	    if (spentPoints > 0) {
		// show minus button
		StringEntity minusButton = new StringEntity((getX() + FIFTH_LEFT_OFFSET) - 25, getY() + (counter1 * PANEL_GAP), "-");
		...
		// show plus button
		StringEntity plusButton = new StringEntity(getX() + FIFTH_LEFT_OFFSET + 25, getY() + (counter1 * PANEL_GAP), "+");
	...
	StringEntity pointsLabel = new StringEntity((getX() + THIRD_LEFT_OFFSET) - 150, getY() + (counter1 * PANEL_GAP), "Available points:");
	...
	if (!StringUtils.isBlank(pc.getCareer())) {
	    StringEntity nextButton = new StringEntity((getX() + getWidth()) - (getWidth() / 4), (getY() + getHeight()) - (getHeight() / 4), 12, "Next");
